...
window.onload = function() {
    ...
    document.getElementById('Rank').addEventListener("click",filterByRank);
    document.getElementById('Id').addEventListener("click",filterById);
    document.getElementById('Title').addEventListener("click",filterByTitle);
...
function filterByTitle(){
    displayMovies(sortMoviesByAttr(movies, 'title'));
}
...
function displayMovies(movies){
    let table = "<table border='5' style='width: 100%'>";
...
function getMaxMovieObject(movies, start, sortAttr){
  ...
  let max_obj = numbers[start];
  let max_location = start

  for (let i = start; i < numbers.length; i++) {
      if (numbers[i] > max_obj) {
          max_obj = numbers[i]
          max_location = i
      }
  }
